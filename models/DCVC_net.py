...
class DCVC_net(nn.Module):
    ...
    def motioncompensation(self, ref, mv):
        ##
        # ref_tmp = ref.squeeze().permute(1,2,0)
        # plt.imshow(ref_tmp)
        # pylab.show()
        ##
    ...
    def forward(self, referframe, input_image, flag = 0):
        ...
        quant_noise_feature_z_mv = torch.nn.init.uniform_(torch.zeros_like(z_mv), -0.5, 0.5)

        if flag == 1:
            compressed_z_mv = torch.round(z_mv)
        else:
            compressed_z_mv = z_mv + quant_noise_feature_z_mv
        params_mv = self.mvpriorDecoder(compressed_z_mv)

        quant_noise_feature_mv = torch.nn.init.uniform_(torch.zeros_like(mvfeature), -0.5, 0.5)
        if flag == 1:
            quant_mv = torch.round(mvfeature)
        else:
            quant_mv = mvfeature + quant_noise_feature_mv
        ...
        quant_noise_feature_z = torch.nn.init.uniform_(torch.zeros_like(z), -0.5, 0.5)
        if flag == 1:
            compressed_z = torch.round(z)
        else:
            compressed_z = z + quant_noise_feature_z
        params = self.priorDecoder(compressed_z)

        feature_renorm = feature

        quant_noise_feature_y_renorm = torch.nn.init.uniform_(torch.zeros_like(feature_renorm), -0.5, 0.5)
        if flag == 1:
            compressed_y_renorm = torch.round(feature_renorm)
        else:
            compressed_y_renorm = feature_renorm + quant_noise_feature_y_renorm
